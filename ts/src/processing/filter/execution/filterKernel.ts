...
import { createValueMatcher, isNegatedOperator, matchesNull, normalizeComparable, type ColumnarComparisonOperator, type ValueComparisonOperator } from "../filterUtils";
...
export function resolveVectorExecutor(vector: Vector, operator: ColumnarComparisonOperator, values: readonly unknown[]): ColumnarVectorExecutor{
...
function resolveDictionaryExecutor(
    ...
    // Two separate questions that used to share one flag: whether the positive match is inverted,
    // and whether a row holding no value matches. They diverge as soon as the operand is null.
    const negated = isNegatedOperator(operator);
    const nullMatches = matchesNull(operator, values);
    return (selection) => scanVector(vector, (index) => {
        if (!vector.has(index)) return nullMatches;
...
function resolveFlatStringExecutor(
    ...
    // As in the dictionary kernel: inverting the byte match and answering for a row that holds no
    // value are distinct, because a null operand flips the second without flipping the first.
    const negated = isNegatedOperator(operator);
    const nullMatches = matchesNull(operator, values);
    return (selection) => scanVector(vector, (index) => {
        if (!vector.has(index)) return nullMatches;
