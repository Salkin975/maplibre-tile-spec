...
describe("FlatGeometryVector", () => {
    ...
    it("filter should return appropriate SelectionVector based on geometry matches", () => {
        ...
        const noTypeMatch = allPoints.filter(SINGLE_PART_GEOMETRY_TYPE.POLYGON);
        expect(noTypeMatch).toBeInstanceOf(ConstSelectionVector);
        expect(noTypeMatch.limit).toBe(0);
