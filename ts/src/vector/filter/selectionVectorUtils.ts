...
/**
 * Selects the indices in `[0, size)` for which `matches` returns true.
 *
 * Without an incoming `selection`, scans fresh and collapses to a `ConstSelectionVector` when every
 * (or no) index matched. With one, narrows it in place instead of scanning `[0, size)` — the caller
 * already knows only those indices are still live.
 */
