...
function scalarColumn(name: string): Column {
    return { name, nullable: false, type: "scalarType", scalarType: { physicalType: ScalarType.INT_32, type: "physicalType" } };
}
...
describe("LazyPropertyVectors", () => {
    ...
    it("skips a column already decoded by an earlier get() while searching for another name", () => {
        ...
        const store = new LazyPropertyVectors(concatenateBuffers(a, b), [pendingColumn("a", 0), pendingColumn("b", a.length)], 1);
